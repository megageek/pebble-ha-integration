"""
API Client for pebble_ha_integration.
...
class PebbleWatchApiClientError(Exception):
    """Base exception to indicate a general API error."""


class PebbleWatchApiClientCommunicationError(
    PebbleWatchApiClientError,
):
    """Exception to indicate a communication error with the API."""


class PebbleWatchApiClientAuthenticationError(
    PebbleWatchApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        PebbleWatchApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise PebbleWatchApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class PebbleWatchApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            PebbleWatchApiClientAuthenticationError: If authentication fails.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise PebbleWatchApiClientCommunicationError(
                msg,
            ) from exception
            ...
            raise PebbleWatchApiClientError(
                msg,
            ) from exception
