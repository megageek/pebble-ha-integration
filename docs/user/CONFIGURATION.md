...
This document describes all configuration options and settings available in the Pebble Watch custom integration.
...
### `pebble_ha_integration.example_service`
...
```yaml
service: pebble_ha_integration.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: pebble_ha_integration.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
  ...
  input:
    sensor_entity:
      ...
      selector:
        entity:
          domain: sensor
          integration: pebble_ha_integration
    threshold:
      name: Threshold
      selector:
        number:
...
- [GitHub Issues](https://github.com/megageek/pebble-ha-integration/issues) - Report problems
