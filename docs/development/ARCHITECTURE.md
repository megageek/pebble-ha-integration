...
This document describes the technical architecture of the Pebble Watch custom component for Home Assistant.

## Directory Structure

```text
custom_components/pebble_ha_integration/
...
│   ├── __init__.py          # Exports PebbleWatchDataUpdateCoordinator
...
│   ├── __init__.py          # Exports PebbleWatchEntity
...
**Key class:** `PebbleWatchDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `PebbleWatchApiClient`
...
- `PebbleWatchConfigFlowHandler` (main flow)
- `PebbleWatchOptionsFlow` (options)
...
**Key class:** `PebbleWatchEntity` (in `entity/base.py`)
...
2. `PebbleWatchEntity` for common functionality
...
1. Create directory: `custom_components/pebble_ha_integration/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `PebbleWatchEntity`
